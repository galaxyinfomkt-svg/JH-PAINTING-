              ...
              {region.painPoints.map((point, idx) => (
                <div key={idx} style={{ background: '#fff', borderRadius: '20px', padding: '2rem 2.5rem', border: '1px solid rgba(0,0,0,0.06)', boxShadow: '0 4px 24px rgba(0,0,0,0.05)' }}>
                  <h3 style={{ fontSize: '1.25rem', fontWeight: 700, color: '#0F172A', marginBottom: '1.25rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: '36px', height: '36px', background: 'linear-gradient(135deg, #C8102E, #9B0A1E)', color: '#fff', borderRadius: '10px', fontSize: '0.875rem', fontWeight: 800, flexShrink: 0 }}>{idx + 1}</span>
                    {point.title}
                  </h3>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '2rem' }}>
                    <div>
                      <p style={{ fontSize: '0.75rem', fontWeight: 700, color: '#C8102E', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.5rem' }}>The Problem</p>
              ...
              {services.map((svc) => (
                <Link key={svc.slug} href={`/regions/${region.slug}/${svc.slug}`} style={{ display: 'block', background: '#fff', borderRadius: '20px', overflow: 'hidden', border: '1px solid rgba(0,0,0,0.04)', boxShadow: '0 4px 24px rgba(0,0,0,0.06)', transition: 'all 0.4s ease', textDecoration: 'none' }}>
                  ...
                  <div style={{ padding: '1.5rem' }}>
                    ...
                    <span style={{ color: '#C8102E', fontWeight: 600, fontSize: '0.875rem', display: 'inline-flex', alignItems: 'center', gap: '0.25rem' }}>Learn More <ChevronRightIcon size={16} /></span>
                  </div>
                </Link>
              ))}
            </div>
