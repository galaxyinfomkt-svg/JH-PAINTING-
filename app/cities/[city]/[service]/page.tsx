...
const CheckCircle2 = ({ size = 20 }: { size?: number }) => (
...
const Star = ({ size = 20, fill }: { size?: number; fill?: string }) => (
...
const Play = ({ size = 20, fill, color }: { size?: number; fill?: string; color?: string }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill={fill || "none"} stroke={color || "currentColor"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="6 3 20 12 6 21 6 3"/></svg>
)
                ...
                {uniqueServiceContent.faq.map((item, idx) => (
                  <details key={idx} className="service-faq-item" style={{
                    ...
                    <summary style={{
                      ...
                      {item.question}
                      <span style={{ flexShrink: 0, marginLeft: '1rem' }}><ChevronRight size={18} /></span>
                    </summary>
                    <p style={{
                      marginTop: '1rem',
                      color: '#555',
