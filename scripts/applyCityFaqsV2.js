// Script to apply generated FAQs to cityContent.ts - V2 more robust
const fs = require('fs');
...
const citiesWithCompleteFaqs = [
  ...
  'worcester', 'cambridge', 'brookline', 'somerville', 'lowell' // Adding more major cities
];
...
// Count FAQs in a string
function countFaqs(faqString) {
  const matches = faqString.match(/question:/g);
    ...
    if (cityFaqs[slug]) {
      ...
      while (i < lines.length) {
        ...
        if (currentLine.match(/^\s*'\w+(-\w+)*':\s*\{/) || currentLine.match(/^\s*\};\s*$/)) {
          // We've reached the next city or end of map, stop here
          break;
        }
